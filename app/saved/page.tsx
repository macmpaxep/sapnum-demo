...
export default async function SavedPage() {
  ...
  return (
    <div>
      <h1 className="text-lg font-semibold text-neutral-900 dark:text-paper">Сохранённые записи</h1>

      {saved.length === 0 && <p className="mt-6 text-sm text-neutral-500 dark:text-neutral-400">У вас пока нет сохранённых записей.</p>}

      <div className="mt-4 space-y-3">
        {saved.map((s) => (
          <article key={s.id} className="rounded-lg border border-neutral-200 dark:border-line p-4">
            <div className="text-sm font-medium text-neutral-900 dark:text-paper">{s.author}</div>
