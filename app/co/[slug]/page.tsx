    ...
    <div className="space-y-6">
      <div className="border border-neutral-200 dark:border-neutral-800 p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold text-neutral-900 dark:text-neutral-50">{company.name}</h1>
      ...
      {!isOwnProfile && (
        <div>
          <h2 className="mb-2 text-sm font-medium text-neutral-900 dark:text-neutral-50">Подать заявку</h2>
      ...
      {company.isOwnerOrAdmin && applications.length > 0 && (
        <div>
          <h2 className="mb-2 text-sm font-medium text-neutral-900 dark:text-neutral-50">Входящие заявки</h2>
      ...
      <div>
        <h2 className="mb-2 text-sm font-medium text-neutral-900 dark:text-neutral-50">Каталог</h2>
        <CatalogManager
      ...
      <div>
        <h2 className="mb-2 text-sm font-medium text-neutral-900 dark:text-neutral-50">Записи компании</h2>
        {posts.length === 0 && <p className="text-sm text-neutral-500 dark:text-neutral-400">Пока нет записей.</p>}
        <div className="space-y-3">
          {posts.map((p) => (
            <article key={p.id} className="border border-neutral-200 dark:border-neutral-800 p-4">
              <div className="flex items-center justify-between text-xs text-neutral-400 dark:text-neutral-500">
                <span className="font-medium text-neutral-900 dark:text-neutral-50">{p.author}</span>
                <span>{p.time}</span>
              </div>
